module id (
    ...
    // 分支指令中 offset 左移两位，再符号扩展至 32 位的值
        ...
        if (rst == `RstEnable) begin
            ...
            case (op)
                ...
                `EXE_LUI: begin                     // lui 指令
                    wreg_o      <=  `WriteEnable;
                    aluop_o     <=  `EXE_OR_OP;     // 底层是通过 ori 来执行的
                    alusel_o    <=  `EXE_RES_LOGIC;
                    reg1_read_o <=  1'b1;
                    reg2_read_o <=  1'b0;
            ...
            if (inst_i[31:21] == 11'b010_0000_0000 && inst_i[10:0] == 11'b00000000000) begin
                aluop_o         <=  `EXE_MFC0_OP;
                alusel_o        <=  `EXE_RES_MOVE;
                wd_o            <=  inst_i[20:16];
                wreg_o          <=  `WriteEnable;
                ...
                reg1_read_o     <=  1'b0;
                reg2_read_o     <=  1'b0;
            end else if (inst_i[31:21] == 11'b010_0000_0100 && inst_i[10:0] == 11'b00000000000) begin
                aluop_o         <=  `EXE_MTC0_OP;
                alusel_o        <=  `EXE_RES_NOP;
                wreg_o          <=  `WriteDisable;
                instvalid       <=  `InstValid;
    ...
    always @(*) begin
        ...
        if (rst == `RstEnable) begin
            ...
            reg1_o <= ex_wdata_i;      // 执行阶段写的 reg1 和现在要读的 reg1 编号相同
        end else if((reg1_read_o == 1'b1) && (mem_wreg_i == 1'b1) && (mem_wd_i == reg1_addr_o)) begin
            reg1_o <= mem_wdata_i;      // 写回阶段写的 reg1 和现在要读的 reg1 编号相同
        end else if (reg1_read_o == 1'b1) begin
            reg1_o <= reg1_data_i;      // 读寄存器
    ...
    always @(*) begin
        ...
        if (rst == `RstEnable) begin
            ...
            reg2_o <= ex_wdata_i;      // 执行阶段写的 reg2 编号和现在要读的一致
        end else if ((reg2_read_o == 1'b1) && (mem_wreg_i == 1'b1) && (mem_wd_i == reg2_addr_o)) begin
            reg2_o <= mem_wdata_i;      // 写回阶段写的 reg2 编号和现在要读的一致
        end else if (reg2_read_o == 1'b1) begin
            reg2_o <= reg2_data_i;      // 读寄存器 
